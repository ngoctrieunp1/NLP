...
MIN_FREQ = 1                 # Tần suất tối thiểu để từ vào vocab
MAX_VOCAB_SIZE = 10_000      # Giới hạn vocab (gợi ý của thầy)
...
MAX_LEN = 50                 # Độ dài tối đa khi dịch
...
def build_vocab(sentences: List[str], tokenizer):
    ...
    # Giới hạn kích thước vocab (nếu lớn hơn MAX_VOCAB_SIZE)
    if len(vocab) > MAX_VOCAB_SIZE:
        print(f"  > Trimming vocab from {len(vocab)} to {MAX_VOCAB_SIZE}")
        all_tokens = vocab.get_itos()
        # Bỏ 4 token đặc biệt ở đầu, lấy tới MAX_VOCAB_SIZE
...
def collate_fn(batch):
    ...
    - Sort batch theo độ dài src giảm dần (enforce_sorted=True cho pack_padded_sequence)
    - Pad src & tgt
    """
    src_seqs, tgt_seqs = zip(*batch)

    src_lengths = torch.tensor([len(s) for s in src_seqs], dtype=torch.long)
    sorted_idx = torch.argsort(src_lengths, descending=True).tolist()
...
class Encoder(nn.Module):
    ...
    def forward(self, src, src_lengths):
        ...
        # Với mô hình không attention, ta chỉ cần (hidden, cell) cuối làm context vector
        return hidden, cell
...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden, cell):
        """
        input: [batch] các index token ở bước t-1
...
class EncoderDecoder(nn.Module):
    ...
    def translate(self, sentence: str, src_tokenizer, src_vocab, tgt_vocab, max_len=50):
        ...
        src_len = torch.tensor([src_tensor.size(0)], dtype=torch.long)

        with torch.no_grad():
            hidden, cell = self.encoder(src_tensor, src_len)
...
def main():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            print("  => Saved new best model to best_model.pth")
        else:
            patience_counter += 1
    ...
    # ===== VẼ SƠ ĐỒ LOSS SAU KHI TRAIN XONG =====
    ...
    print(f"Test BLEU score (mean sentence BLEU): {bleu:.4f}")


# Hàm translate(sentence: str) bắt buộc (dùng model đã train xong)
def translate(sentence: str) -> str:
    """
    API đơn giản cho đề bài:
        def translate(sentence: str) -> str

    Lưu ý: hàm này giả định đã có file best_model.pth trong thư mục hiện tại
    (được tạo sau khi chạy main()).
    """
    checkpoint = torch.load("best_model.pth", map_location=DEVICE)
    ...
    # Khởi tạo lại model đúng cấu hình khi train
